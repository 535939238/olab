<template>
    <div id='MainEditor' class="slidepanel">
        <button class="close" @click="$emit('goback')">&times;</button>
        <p class="pretitle">{{data.title}}</p>
        <div class="imgcontainer">
            <a :href="data.image" target="#" :style="'background-image:url('+data.image+')'"></a>
        </div>
        <span class="label label-default">标题</span>
        <input type="text" class="form-control" v-model="data.title" :disabled="data.bindpage">
        <span class="label label-default">链接</span>
        <div class="input-group">
            <input type="text" class="form-control" v-model="data.url" :disabled="data.bindpage">
            <span class="input-group-addon">
                <div class="checkbox">
                    <label>
                        <input type="checkbox" v-model="data.bindpage"> 绑定页面
                    </label>
                </div>
            </span>
        </div>
        <span class="label label-default">图片</span>
        <div class="input-group">
            <input type="text" class="form-control" v-model="data.image" :disabled="data.bindpage">
            <span class="input-group-addon">
                <div class="altbtn altbtn-info" @click="BindImage" v-if="!data.bindpage">
        ...
        <input type="text" class="form-control" v-model="data.contains" :disabled="data.bindpage">
        <div class="flexdep">
            <div>
                <div class="btn btn-danger btn-block" @click="$emit('delete')">删除</div>
            </div>
        </div>
    </div>
</template>
  ...
  methods: {
    BindImage() {
      modal.Show({
        ...
        SubComp: ImageSelect,
        SubCompEvent: res => {
          this.data.image = `/static/uploads/${res}`;
          modal.show = false;
        }
      });
    }
    ...
    "data.bindpage"(bind) {
      if (bind == true) {
        modal.Show({
          ...
          SubComp: PageSelect,
          SubCompEvent: index => {
            axios.get(`/setting/getpage/id/${index}`).then(res => {
              res = res.data;
              this.data.contains = res.contains;
              this.data.url = `/page/${index}`;
              this.data.image = res.image;
              this.data.bind = index;
              this.$set(this.data, "title", res.title);
              modal.show = false;
            });
          },
          cancelled: ()=>{
              this.data.bindpage = false;
          }
        });
      }
    }
